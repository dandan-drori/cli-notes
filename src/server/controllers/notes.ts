...
import {
	...
	getNoteById,
	save,
	moveNoteToTrash as moveNoteToTrashDb,
	unlockNote as unlockNoteDb,
	lockNote as lockNoteDb,
} from '../../db/mongo';
...
export async function unlockNote(req: Request, res: Response) {
	try {
		const { note, password } = req.body;
		const result = await unlockNoteDb(note, password);
		res.status(200).json(result);
	} catch (e) {
		console.log('Error: failed to unlock note');
		res.status(500).json(e);
	}
}

export async function lockNote(req: Request, res: Response) {
	try {
		const { note, password } = req.body;
		const result = await lockNoteDb(note, '', password);
		res.status(200).json(result);
	} catch (e) {
		console.log('Error: failed to unlock note');
		res.status(500).json(e);
